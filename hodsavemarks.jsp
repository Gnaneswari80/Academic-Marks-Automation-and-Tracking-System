...
<%! 
public int parseMark(String val) {
    if(val == null || val.trim().isEmpty()) {
        return 0; // treat empty as 0
    }
    if(val.equalsIgnoreCase("Absent")) {
        return -1; // store -1 when Absent
    }
    try {
    ...
    for(String mid : mids){
        for(Map<String,String> stu : students){
         ...
         // 1️⃣ Check if record exists
            ...
            Map<String,Integer> existingMarks = new HashMap<>();
            if(exists){
                // Fetch existing marks to preserve -1
                String fetchSql = "SELECT * FROM studentmidmarks WHERE sid=? AND smmMid=? AND sub_id=? AND student_sem=? AND student_year=? AND sbatch_year=?";
                PreparedStatement fetchPs = con.prepareStatement(fetchSql);
                fetchPs.setString(1, sid);
                fetchPs.setString(2, mid);
                fetchPs.setString(3, subjectId);
                fetchPs.setString(4, sem);
                fetchPs.setString(5, year);
                fetchPs.setString(6, batchYear);
                ResultSet rsMarks = fetchPs.executeQuery();
                if(rsMarks.next()) {
                    for(int i=1;i<=3;i++) existingMarks.put("marks_obj_" + i, rsMarks.getInt("marks_obj_" + i));
                    for(int i=1;i<=3;i++) existingMarks.put("marks_ass_" + i, rsMarks.getInt("marks_ass_" + i));
                    for(int q=1;q<=6;q++){
                        for(char sub='a';sub<='b';sub++){
                            String col = "marks_" + q + sub;
                            existingMarks.put(col, rsMarks.getInt(col));
                        }
                    }
                }
                rsMarks.close();
            ...
            if(exists){
                ...
                // Set OBJ
                for(int i=1;i<=3;i++){
                    String param = mid+"_obj"+i+"_"+sid;
                    int dbMark = existingMarks.getOrDefault("marks_obj_" + i, 0);
                    int formMark = parseMark(request.getParameter(param));
                    int finalMark = (dbMark == -1) ? -1 : (formMark == -1 ? 0 : formMark);
                    ps.setInt(index++, finalMark);
                }

                // Set ASS
                for(int i=1;i<=3;i++){
                    String param = mid+"_ass"+i+"_"+sid;
                    int dbMark = existingMarks.getOrDefault("marks_ass_" + i, 0);
                    int formMark = parseMark(request.getParameter(param));
                    int finalMark = (dbMark == -1) ? -1 : (formMark == -1 ? 0 : formMark);
                    ps.setInt(index++, finalMark);
                }
                ...
                for(int q=1;q<=6;q++){
                    for(char sub='a';sub<='b';sub++){
                        ...
                        int dbMark = existingMarks.getOrDefault(col, 0);
                        int formMark = parseMark(request.getParameter(param));
                        int finalMark = (dbMark == -1) ? -1 : (formMark == -1 ? 0 : formMark);
                        ps.setInt(index++, finalMark);
                    }
                ...
                // 3️⃣ INSERT (no preservation needed)
                ...
                // Identifier columns
                ps.setString(index++, sid);
                ps.setString(index++, stu.get("student_name"));
                ps.setString(index++, subjectId);
                ps.setString(index++, sem);
                ps.setString(index++, year);
                ps.setString(index++, batchYear);
                ...
                for(int i=1;i<=3;i++){
                    String param = mid+"_obj"+i+"_"+sid;
                    ps.setInt(index++, parseMark(request.getParameter(param)));
                }
...
window.location.href = "hodviewmarks.jsp"; </script>
