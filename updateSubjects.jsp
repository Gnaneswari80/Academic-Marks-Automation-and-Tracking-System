...
<%
    ...
    try {
        ...
        // Load faculty list (case-insensitive, same department)
        facStmt = conn.prepareStatement(
            "SELECT faculty_id, faculty_name, faculty_department FROM faculty WHERE LOWER(faculty_department) = LOWER(?)"
        );
        ...
        // Load subjects (Admin & HOD see only their department)
...
<head>
    ...
    <style>
        table { width: 95%; border-collapse: collapse; margin: 30px auto; }
        th, td { border: 1px solid #ccc; padding: 10px; text-align: center; }
        th { background-color: #f2f2f2; }
        input[type="text"], select { width: 100%; padding: 5px; }
        input[type="submit"], a.delete-button {
            padding: 6px 12px; border: none; border-radius: 5px; color: white; cursor: pointer;
        }
        input[type="submit"] { background-color: #4CAF50; }
        a.delete-button { background-color: #f44336; text-decoration: none; }
        a.delete-button:hover { background-color: #d32f2f; }
        .nav-home { margin: 20px auto; text-align: center; }
        .nav-home a {
            background-color: #2196F3; padding: 10px 20px; color: white;
            text-decoration: none; border-radius: 5px;
        }
        .nav-home a:hover { background-color: #0b7dda; }
    </style>
</head>
<body>
            ...
            <td>
                <select name="faculty">
                    ...
                    <% for (String[] faculty : facultyList) {
                        ...
                        String selected = (fid != null && optionValue.trim().equalsIgnoreCase(fid.trim())) ? "selected" : "";
                    %>
